...
offer_uri=$(curl -s -k -X GET "$1/realms/test-realm/protocol/oid4vc/credential-offer-uri?credential_configuration_id=$2" \
  --header "Authorization: Bearer ${access_token}" | jq '"\(.issuer)\(.nonce)"' -r)

export pre_authorized_code=$(curl -s -k -X GET ${offer_uri} \
...
credential_access_token=$(curl -s -k -X POST "$1/realms/test-realm/protocol/openid-connect/token" \
  --header 'Accept: */*' \
  --header 'Content-Type: application/x-www-form-urlencoded' \
  --data grant_type=urn:ietf:params:oauth:grant-type:pre-authorized_code \
  --data pre-authorized_code=${pre_authorized_code} | jq '.access_token' -r)

curl -s -k -X POST "$1/realms/test-realm/protocol/oid4vc/credential" \
  --header 'Accept: */*' \
