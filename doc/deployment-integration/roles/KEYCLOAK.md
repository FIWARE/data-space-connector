...
> :warning: Since Keycloak 26.6.4 the `java-keystore` realm key provider only reads keystores from
> `${kc.home.dir}/data/<realm-name>` (`/opt/keycloak/data/<realm-name>` on the official image). The
> mount path below must therefore end in the realm name configured under `realm.name` — `test-realm`
> in this example. Keystores outside that boundary make Keycloak reject any create/update on the key
> provider.
...
```yaml
  ...
  signingKey:
    storePath: /opt/keycloak/data/test-realm/cert.pfx   # PKCS12 keystore (from init container); dir must be the realm name
...
The chart ships the upstream `keycloak/keycloak:26.7.0` image. Two things to know:

- **Database migration:** 26.7.0 carries the Liquibase changeset `26.7.0-verifiable-credential`
  (adds the `version` column to the custom `USER_VER_CREDENTIAL` table), so reusing a pre-26.4
  database is safe. Only images older than 26.6.3 fail at issuance with
  `column ... version does not exist`. Also remember Keycloak does **not** re-import an existing
  realm — recreate the realm to apply `realm.*` changes.
