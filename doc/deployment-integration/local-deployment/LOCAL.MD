...
In order to support development and exploration of the FIWARE Data Space Connector, a "Minimum Viable Dataspace" is
...
> **Note:** The Local Deployment is for **learning and development purposes only**. It deploys all the components of a data space together to show how they interact, not from the perspective of any specific role. For deploying the FIWARE DSC in a real environment, refer to the documentation that matches your organization's role in the data space: [Deployment by Role](../roles/README.md).
...
To check if the tools are installed, can run and are able to spin up a kubernetes cluster without further FIWARE components, you should execute the prepared [script](checkRequirements.sh).
...
> :warning: The local deployment uses [k3s](https://k3s.io/) in a docker container and is currently only tested on linux and MacOS.
...
for Issuers and storage in a PSQL Database. After deployment, the API is available
...
```shell
  curl -k -x localhost:8888 -X POST https://til.127.0.0.1.nip.io/issuer \
...
The Minimum Data Space should provide an easy-to-understand introduction to the FIWARE Data Space. Therefor the roles of
the
...
```shell
  curl -k -x localhost:8888 -X GET https://keycloak-consumer.127.0.0.1.nip.io/realms/test-realm/.well-known/openid-credential-issuer | jq .
```

Get a credential offer uri(for the `user-credential), using the retrieved AccessToken:

```shell
  export OFFER_URI=$(curl -s -k -x localhost:8888 -X GET 'https://keycloak-consumer.127.0.0.1.nip.io/realms/test-realm/protocol/oid4vc/create-credential-offer?credential_configuration_id=user-credential&pre_authorized=true' \
...
```shell
  token_response=$(curl -s -k -x localhost:8888 -X POST https://keycloak-consumer.127.0.0.1.nip.io/realms/test-realm/protocol/openid-connect/token \
    --header 'Accept: */*' \
    --header 'Content-Type: application/x-www-form-urlencoded' \
    --data grant_type=urn:ietf:params:oauth:grant-type:pre-authorized_code \
    --data pre-authorized_code=${PRE_AUTHORIZED_CODE})
  export CREDENTIAL_ACCESS_TOKEN=$(echo $token_response | jq '.access_token' -r); echo ${CREDENTIAL_ACCESS_TOKEN}
  export CREDENTIAL_IDENTIFIER=$(echo "${token_response}" | jq -r '
    .authorization_details[]? |
    select(.credential_identifiers != null and (.credential_identifiers | length) > 0) |
    .credential_identifiers[0]' | head -n 1); echo ${CREDENTIAL_IDENTIFIER}
```
...
```shell
  export VERIFIABLE_CREDENTIAL=$(curl -s -k -x localhost:8888 -X POST https://keycloak-consumer.127.0.0.1.nip.io/realms/test-realm/protocol/oid4vc/credential \
    ...
    --data "{
            ...
            }" | jq '.credentials[0].credential' -r); echo ${VERIFIABLE_CREDENTIAL}
```
...
![Provider](../../img/local/provider.png)
...
an ingress at ```https://scorpio-provider.127.0.0.1.nip.io/ngsi-ld/v1```. In
...
```shell
  curl -k -x localhost:8888 -s -X GET 'https://mp-data-service.127.0.0.1.nip.io/ngsi-ld/v1/entities/urn:ngsi-ld:EnergyReport:fms-1'
```

The normal flow is now to request the oidc-information at the well-known endpoint:

```shell
  export TOKEN_ENDPOINT=$(curl -k -x localhost:8888 -s -X GET 'https://mp-data-service.127.0.0.1.nip.io/.well-known/openid-configuration' | jq -r '.token_endpoint'); echo $TOKEN_ENDPOINT
```
...
Create the signature. Note: OpenSSL produces ECDSA signatures in DER format, but JWS requires raw R||S format (64 bytes for P-256). The `convert_ec` function from `doc/scripts/convert_ec.sh` handles this conversion:

```shell
  source doc/scripts/convert_ec.sh
  export SIGNATURE=$(echo -n "${JWT_HEADER}.${PAYLOAD}" | openssl dgst -sha256 -binary -sign cert/private-key.pem | convert_ec | base64 -w0 | sed s/\+/-/g | sed 's/\//_/g' | sed -E s/=+$//); echo Signature: ${SIGNATURE};
```
...
An essential use-case within Dataspaces is trading of data and digital services. The Minimum Viable Dataspace supports
...
2. Create the product specification. The specification should contain the configuration of credentials to be trusted by the customer(in this case, "OperatorCredential" with the role "OPERATOR" for the target-service) and the policy to be created for the customer. In the example, we will offer 2 different types service: "small" and "full", where small restricts the cluster-size to be created:
  ...
  1. "small":
  ```shell
  export PRODUCT_SPEC_SMALL_ID=$(curl -k -x localhost:8888 -X 'POST' https://tm-forum-api.127.0.0.1.nip.io/tmf-api/productCatalogManagement/v4/productSpecification \
  ...
  1. "full":
  ```shell
  export PRODUCT_SPEC_FULL_ID=$(curl -k -x localhost:8888 -X 'POST' https://tm-forum-api.127.0.0.1.nip.io/tmf-api/productCatalogManagement/v4/productSpecification \
    -H 'Content-Type: application/json;charset=utf-8' \
    -d "{
      \"brand\": \"M&P Operations\",
      \"version\": \"1.0.0\",
...
3. Create a product offering, referencing the spec:
  ...
  1. "small":
  ```shell
  export PRODUCT_OFFERING_SMALL_ID=$(curl -k -x localhost:8888 -X 'POST' https://tm-forum-api.127.0.0.1.nip.io/tmf-api/productCatalogManagement/v4/productOffering \
    -H 'Content-Type: application/json;charset=utf-8' \
    -d "{
      \"version\": \"1.0.0\",
      \"lifecycleStatus\": \"ACTIVE\",
  ...
  2. "small":
  ```shell
  export PRODUCT_OFFERING_FULL_ID=$(curl -k -x localhost:8888 -X 'POST' https://tm-forum-api.127.0.0.1.nip.io/tmf-api/productCatalogManagement/v4/productOffering \
    -H 'Content-Type: application/json;charset=utf-8' \
    -d "{
      \"version\": \"1.0.0\",
      \"lifecycleStatus\": \"ACTIVE\",
...
```shell
  ...
  curl -k -x localhost:8888 -X POST https://mp-data-service.127.0.0.1.nip.io/ngsi-ld/v1/entities \
    -H 'Accept: */*' \
    -H 'Content-Type: application/json' \
...
```shell
    ./doc/scripts/get_access_token_oid4vp.sh https://mp-data-service.127.0.0.1.nip.io $OPERATOR_CREDENTIAL operator
```

> :bulb: For easier access, the identity(in form of the did) of the consumer is available at ```https://did-consumer.127.0.0.1.nip.io/did-material/did.env```.
...
```shell
  ...
  export FANCY_MARKETPLACE_ID=$(curl -k -x localhost:8888 -X POST https://mp-tmf-api.127.0.0.1.nip.io/tmf-api/party/v4/organization \
    -H 'Accept: */*' \
    -H 'Content-Type: application/json' \
    -H "Authorization: Bearer ${ACCESS_TOKEN}" \
    -d "{
...
with the TMForum-APIs directly.
  ...
  ```shell
    export ACCESS_TOKEN=$(./doc/scripts/get_access_token_oid4vp.sh https://mp-data-service.127.0.0.1.nip.io $REP_CREDENTIAL default); echo $ACCESS_TOKEN
    curl -k -x localhost:8888 -X GET https://mp-tmf-api.127.0.0.1.nip.io/tmf-api/productCatalogManagement/v4/productOffering -H "Authorization: Bearer ${ACCESS_TOKEN}" | jq .
  ```
  2. To accept the offering, one has to be choosen and its id needs to be extracted. We will start with the "small" version:
  ```shell
    export ACCESS_TOKEN=$(./doc/scripts/get_access_token_oid4vp.sh https://mp-data-service.127.0.0.1.nip.io $REP_CREDENTIAL default); echo $ACCESS_TOKEN
    export OFFER_ID=$(curl -k -x localhost:8888 -X GET https://mp-tmf-api.127.0.0.1.nip.io/tmf-api/productCatalogManagement/v4/productOffering -H "Authorization: Bearer ${ACCESS_TOKEN}" | jq '.[0].id' -r); echo ${OFFER_ID}
  ```
  3. Create the order:
  ```shell
    export ACCESS_TOKEN=$(./doc/scripts/get_access_token_oid4vp.sh https://mp-data-service.127.0.0.1.nip.io $REP_CREDENTIAL default); echo $ACCESS_TOKEN
    export ORDER_ID=$(curl -k -x localhost:8888 -X POST https://mp-tmf-api.127.0.0.1.nip.io/tmf-api/productOrderingManagement/v4/productOrder \
    -H 'Accept: */*' \
    -H 'Content-Type: application/json' \
    -H "Authorization: Bearer ${ACCESS_TOKEN}" \
    -d "{
...
```shell
  export ACCESS_TOKEN=$(./doc/scripts/get_access_token_oid4vp.sh https://mp-data-service.127.0.0.1.nip.io $OPERATOR_CREDENTIAL operator)
  curl -k -x localhost:8888 -X GET https://mp-data-service.127.0.0.1.nip.io/ngsi-ld/v1/entities/urn:ngsi-ld:K8SCluster:fancy-marketplace \
    -H 'Accept: */*' \
    -H "Authorization: Bearer ${ACCESS_TOKEN}"
```
...
9. Buy access to the full offering:
  ...
  ```shell
      export ACCESS_TOKEN=$(./doc/scripts/get_access_token_oid4vp.sh https://mp-data-service.127.0.0.1.nip.io $REP_CREDENTIAL default); echo $ACCESS_TOKEN
      export OFFER_ID=$(curl -k -x localhost:8888 -X GET https://mp-tmf-api.127.0.0.1.nip.io/tmf-api/productCatalogManagement/v4/productOffering -H "Authorization: Bearer ${ACCESS_TOKEN}" | jq '.[1].id' -r); echo ${OFFER_ID}
  ```
...
```shell
  curl -k -x localhost:8888 -X 'PUT' \
    'https://provider-ccs.127.0.0.1.nip.io/service/data-service' \
    -H 'accept: application/json' \
    -H 'Content-Type: application/json' \
    -d '{
...
```shell
  ./doc/scripts/get_access_token_oid4vp.sh https://mp-data-service.127.0.0.1.nip.io $USER_CREDENTIAL default
```
...
The Data Space Connector supports [eIDAS 2.0](https://digital-strategy.ec.europa.eu/en/policies/eidas-regulation) compliance through the [`did:elsi` DID method](https://alastria.github.io/did-method-elsi/), which ties organization identity to qualified certificates issued under the EU Trust Framework.

Starting with **decentralized-iam >= 2.1.23 / VCVerifier >= 6.22.0**, certificate chain validation is performed natively by VCVerifier using PKIX validation against the [EU Trusted Lists (ETSI TS 119 612)](https://ec.europa.eu/digital-building-blocks/sites/display/DIGITAL/EU+Trusted+Lists). The previously required external `dss-validation-service` is no longer needed.

For full documentation on eIDAS configuration, certificate generation, migration from the DSS-based approach, and troubleshooting, see the **[standalone eIDAS documentation](../eidas/README.md)**.

### Quick Start (Local Deployment)

Once the provider and consumer are configured per the [eIDAS documentation](../eidas/README.md), deploy locally with:
```shell
mvn clean deploy -Plocal,elsi
```

The interaction is the same as the standard local deployment (see [Demo Interactions](#demo-interactions)). The visible difference is the credential format: the JWT's `iss` claim is a `did:elsi` DID and the `x5c` header contains the certificate chain.

See [k3s/provider-elsi.yaml](../../../k3s/provider-elsi.yaml) and [k3s/consumer-elsi.yaml](../../../k3s/consumer-elsi.yaml) for the complete local overlay files.
...
|--------------------------------------------------|--------------------------------------------|----------------------------------------------------------------------------|--------------|-------------------------------------------------------------------------------------|------------------------------------------------------|
| https://tir.127.0.0.1.nip.io:8443/                | https://tir.127.0.0.1.nip.io/               | [Trusted Issuers Registry](https://github.com/FIWARE/trusted-issuers-list) | Trust Anchor | Provides the list of trusted issuers                                                | no                                                   |
| https://til.127.0.0.1.nip.io:8443/                | https://til.127.0.0.1.nip.io/               | [Trusted Issuers List API](https://github.com/FIWARE/trusted-issuers-list) | Trust Anchor | Create,Update,Delete functionality for the Trusted Issuers Registry                 | should not be publicly available in real data spaces |
| https://keycloak-consumer.127.0.0.1.nip.io:8443/  | https://keycloak-consumer.127.0.0.1.nip.io | [Keycloak](https://github.com/keycloak/keycloak)                           | Consumer     | Issues credentials on behalf of the consumer                                        | no                                                   |
...
| https://keycloak-provider.127.0.0.1.nip.io:8443/  | https://keycloak-provider.127.0.0.1.nip.io | [Keycloak](https://github.com/keycloak/keycloak)                           | Provider     | Issues credentials on behalf of the provider                                        | no                                                   |
| http://did-consumer.127.0.0.1.nip.io:8080/       | https://did-consumer.127.0.0.1.nip.io/      | [did-helper](https://github.com/wistefan/did-helper)                       | Consumer     | Helper to provide access to the consumers did and key material                      | yes, should never be public                          |
| https://mp-data-service.127.0.0.1.nip.io:8443/    | http://mp-data-service.127.0.0.1.nip.io/   | [Apisix](https://apisix.apache.org/)                                       | Provider     | ApiGateway to be used as entry point to all secured services, f.e. the data-service | no                                                   |
