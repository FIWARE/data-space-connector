...
| ENF-2 | **Access policy for the new path** so a seller may create and read service specs through the OID4VP-protected API — the analogue of `allowProductSpec.json`. No policy-decision-point change is needed: `tmf:resource` derives the resource from the request path generically. The integration tests author specifications directly against the TMForum API and do not exercise it, so the policy is authored per deployment rather than shipped here | deployment | open |
